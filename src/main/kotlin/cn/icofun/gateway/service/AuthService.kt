...
import cn.icofun.gateway.model.LoginRequest
import cn.icofun.gateway.model.SysRole
import cn.icofun.gateway.model.SysUser
...
import cn.icofun.gateway.model.vo.MetaVo
...
class AuthService(
    ...
    @Transactional
    fun saveUser(user: SysUser): Mono<SysUserEntity> {
    ...
    fun getUserById(id: Long): Mono<SysUser> {
        return userRepository.findById(id)
            .flatMap { userEntity ->
    ...
    fun getUser(username: String): Mono<SysUser> {
        ...
        return userRepository.findAll()
            .flatMap { entity ->
                userRoleRepository.findByUserId(entity.id!!)
                    ...
                    .map { roles ->
                        val sysRoles = roles.map { SysRole(it.id, it.name, it.code) }
                        SysUser(
                            id = entity.id,
    ...
    private fun fetchUserRoles(userEntity: SysUserEntity): Mono<SysUser> {
